# -*- coding: utf-8 -*-
"""UrsuAndrei_gr312_ProiectML.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/16sW-JKhI1hGlI_n6tLAA1E_pEZ1ShYLq
"""
...
def preprocess_nltk(text):
  ...
  tokenized_text = [t for t in tokenized_text if t not in stopwords.words('english')]
  print(tokenized_text)
  return tokenized_text

data = []
for propozitie in propozitii:
  data.append(preprocess_nltk(str(propozitie)))
...
def train_and_test(k, X_train, X_valid, y_train):
 model_knn = KNeighborsClassifier(n_neighbors=k)
 model_knn = model_knn.fit(X_train, y_train)
 predicted_labels_knn = model_knn.predict(X_valid)
 acc = accuracy_score(y_valid, predicted_labels_knn)
 return acc
...
for k in range(1,250):
  acc.append(train_and_test(k,X_train, X_valid, train_labels))
  if max_acc<acc[k-1]:
    max_acc=acc[k-1]
...
print("Cea mai buna acuratete este obtinuta pentru k= ",best_k)
print("Acuratetea este: ",max_acc)
